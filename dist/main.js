...
// src/puzzle/interfaces.ts
var Direction;
(function(Direction2) {
  Direction2[Direction2["LR"] = 0] = "LR";
  Direction2[Direction2["UD"] = 1] = "UD";
  Direction2[Direction2["None"] = 2] = "None";
})(Direction || (Direction = {}));

// src/puzzle/linkedlist.ts
var CharNode = class {
  constructor(index, char, direction) {
    this.index = index;
    this.value = char;
...
var LinkedList = class {
  ...
  addCharToHead(char, direction) {
    if (!this.head && !this.tail) {
      this.createHeadTail(0, char, direction);
    } else {
      let currentHead = this.head;
      let newHead = new CharNode(currentHead.index - 1, char, direction);
  ...
  addToIndex(index, char, direction) {
    let head = this.head;
    let tail = this.tail;
    if (!head && !tail) {
      if (index == 0) {
        ...
        let head2 = this.head;
        let tail2 = this.tail;
        if (index > head2.index && index < tail2.index) {
          for (let i = head2.index; i < index; i++) {
            this.addCharToLast("?", Direction.None);
          }
          this.addCharToLast(char, direction);
        } else if (index > tail2.index) {
          for (let i = tail2.index + 1; i < index; i++) {
            this.addCharToLast("?", Direction.None);
          }
          this.addCharToLast(char, direction);
        }
      ...
      if (index == head.index) {
        ...
        let current = head;
        while (current.next != void 0 && current.index != index)
  ...
  createHeadTail(index, char, direction) {
  ...
  getCharAt(searchIndex) {
    let current = this.tail;
    while (current.prev !== void 0) {
...
var WordDb = class {
  ...
  searchAndAddToDb(word, charIndex) {
    let isAdded = false;
    const charToSearch = word[charIndex];
    if (this.charLists.size > 0) {
        ...
        if (listFound) {
          ...
          if (charFound) {
            if (charFound.direction == Direction.LR) {
              let area2 = this.checkWordAreaEmpty(charFound.index, charIndex, word, charFound.direction, charFound, i, false);
              if (area2 && area2.isEmpty) {
              ...
              let area2 = this.checkWordAreaEmpty(i, charIndex, word, charFound.direction, charFound, i, false);
              if (area2 && area2.isEmpty) {
  ...
  addToIndex(listIndex, charIndex, char, direction) {
  ...
  checkWordAreaEmpty(loopIndex1, loopIndex2, word, direction, charFound, listIndex, add) {
    ...
    for (let j = startIndex, t = 0; j < endIndex, t < wordLength; j++, t++) {
      if (direction == Direction.LR) {
        ...
        if (loopIndex1 == j) {
          ...
          if (!add && (this.hasChar(listIndex - 1, j) || this.hasChar(listIndex + 1, j))) {
            return {isEmpty: false};
          } else {
            if (add && loopIndex1 > j) {
        ...
        if (loopIndex1 == j) {
          ...
          if (!add && (this.hasChar(j, charFound.index - 1) || this.hasChar(j, charFound.index + 1))) {
            return {isEmpty: false};
          } else if (add) {
  ...
  generateKaboomLevel() {
    let chars = this.getCharmap();
    let yMin = import_lodash.default.minBy(chars, "y").y;
    let yMax = import_lodash.default.maxBy(chars, "y").y;
    let xMin = import_lodash.default.minBy(chars, "x").x;
    let xMax = import_lodash.default.maxBy(chars, "x").x;
    ...
    let lines = import_lodash.default.chain(chars).groupBy("y").map((value, key) => ({y: key, chars: value})).value();
    let level = [];
...
// src/puzzle/wordpuzzle.ts
var WordPuzzle = class {
  constructor(words) {
    this.words = [];
  ...
  search(word, charIndex) {
    let wordAdded = this.wordDb.searchAndAddToDb(word, charIndex);
    if (!wordAdded) {
      if (charIndex < word.length - 1)
        return this.search(word, charIndex + 1);
      else if (charIndex == word.length - 1 && !this.unusedWords.includes(word)) {
